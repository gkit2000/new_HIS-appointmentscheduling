...
public class AppointmentBlockValidator implements Validator {
	...
	public void validate(Object obj, Errors errors) {
		...
		if (appointmentBlock == null) {
			...
			//If block added by vishwajeet to remove validation for multiple days.
			if(appointmentBlock.getIsmulti().equalsIgnoreCase("YES")){
				...
				if (Context.getService(AppointmentService.class).getOverlappingAppointmentBlocks(appointmentBlock).size() > 0) {
					...
					errors.rejectValue("provider", "appointmentscheduling.AppointmentBlock.error.appointmentBlockOverlap start date"+appointmentBlock.getStartDate().toString()+"End date"+appointmentBlock.getEndDate());
